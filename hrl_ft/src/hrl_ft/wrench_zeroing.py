...
def process_data(data, is_backwards):
    ...
    if is_backwards:
        ...
        react_mult = 1.
    for w, quat in data:
        wf_chain.extend(w[:3]) #Add force values to 3*Nx1 matrix (N is number of samples)
        rot_mat = np.mat(tf_trans.quaternion_matrix(quat))[:3,:3]
        z_grav = react_mult * rot_mat.T * np.mat([0, 0, -1.]).T #Get ideal direction of gravity in wrench frame 
        z_x, z_y, z_z = z_grav.T.A[0]
        grav_chain.append([g * z_x, 1, 0, 0]) #build 3*Nx4 matrix
